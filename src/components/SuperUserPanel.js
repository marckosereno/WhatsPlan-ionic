...
export class SuperUserPanel {
  ...
  _buildCatChips() {
    ...
    const cats = this.mapView?.categories ||
      window.wpApp?.mapView?.categories || [];
    row.innerHTML = '';
    // Chip "Todas"
    ...
    cats.forEach(cat => {
      ...
      const lang = window.wpApp?.state?.getState('currentLanguage') || 'es';
  ...
  _buildCategoriesPanel() {
    ...
    const colorMap = { yellow:'#f59e0b', green:'#10b981', blue:'#3b82f6', purple:'#6366f1', gray:'#6b7280' };
    colors.forEach((c, i) => {
      const btn = document.createElement('button');
      btn.className = 'su-color-btn' + (i === 0 ? ' active' : '');
      ...
      btn.addEventListener('click', async e => {
        ...
        try {
          ...
          if (mapView) {
            ...
            mapView.allPlaces = mapView.places || [];
            mapView._clearPlaceMarkers();
            mapView._renderPlaceMarkers(mapView.allPlaces);
            const _cr1 = document.getElementById('map-results-count');
            if (_cr1) _cr1.textContent = mapView.allPlaces.length + ' lugares';
          }
    ...
    menu.querySelector('.su-del-confirm-yes').addEventListener('click', async () => {
      ...
      try {
        ...
        if (mapView) {
          ...
          mapView._clearPlaceMarkers();
          mapView._renderPlaceMarkers(mapView.allPlaces);
          const _cr2 = document.getElementById('map-results-count');
          if (_cr2) _cr2.textContent = mapView.allPlaces.length + ' lugares';
        }
  ...
  _openBatchTagPicker(allSubcats, selectedIds, places, onDone) {
    ...
    modal.innerHTML =
      '<div style="width:100%;max-width:480px;background:#1a1a2e;border-radius:20px 20px 0 0;padding:20px;display:flex;flex-direction:column;gap:12px;max-height:80dvh;">' +
        ...
        '<button id="su-btp-apply" style="background:linear-gradient(135deg,#00bcd4,#6366f1);border:none;border-radius:12px;color:#fff;padding:14px;font-size:14px;font-weight:700;cursor:pointer;flex-shrink:0;">✅ Aplicar a ' + selectedPlaces.length + ' lugares</button>' +
      '</div>';
    ...
    document.getElementById('su-btp-apply').addEventListener('click', async () => {
      ...
      // Refrescar mapa con datos ya actualizados en memoria
      const mapView = window.wpApp?.mapView;
      if (mapView) {
        mapView.allPlaces = mapView.places || [];
        mapView._clearPlaceMarkers();
        mapView._renderPlaceMarkers(mapView.allPlaces);
        const _cr3 = document.getElementById('map-results-count');
        if (_cr3) _cr3.textContent = mapView.allPlaces.length + ' lugares';
      }
    });
  }
  ...
  _pickCoordsFromMap(callback) {
    ...
    banner.style.cssText = 'position:fixed;top:0;left:0;right:0;z-index:99999;background:linear-gradient(135deg,#0891b2,#6366f1);color:#fff;padding:16px 20px;display:flex;align-items:center;justify-content:space-between;font-weight:700;font-size:14px;';
  ...
  _injectStyles() {
    ...
    style.textContent = `
      ...
      .su-btn-purple { background: linear-gradient(135deg,#6366f1,#4338ca); color: white; }
      ...
      .su-size-btn.active { background:#6366f1; border-color:#6366f1; color:white; }
