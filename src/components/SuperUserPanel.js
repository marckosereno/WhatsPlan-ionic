...
export class SuperUserPanel {
  ...
  _buildCatChips() {
    ...
    if (!row) return;
    // Obtener categorías disponibles del mapView o de window.himarcoApp
    const cats = this.mapView?.categories ||
      window.himarcoApp?.mapView?.categories || [];
    row.innerHTML = '';
    // Chip "Todas"
    ...
    cats.forEach(cat => {
      ...
      const lang = window.himarcoApp?.state?.getState('currentLanguage') || 'es';
  ...
  async _reloadLandmarks() {
    try {
      ...
      this.mapView.renderLandmarks(items);
  ...
  async _openPlaces() {
    ...
    const mapView = window.himarcoApp?.mapView;
      ...
      btn.addEventListener('click', async e => {
        ...
        try {
          ...
          if (mapView) {
            ...
            mapView.renderMarkers(mapView.places);
            mapView.renderList(mapView.places);
            mapView.updateCount(mapView.places.length);
          }
