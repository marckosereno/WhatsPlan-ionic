  ...
  async showSubcats(menuKey) {
    ...
    this._footerEl.querySelectorAll('.subcategory-footer-chip').forEach(chip => {
      chip.addEventListener('click', (e) => {
        ...
        // Centrado en el scroll horizontal (no solo "lo mínimo para que se
        // vea") — así el chip tocado queda a la vista sin importar si está
        // cerca del borde izquierdo o derecho del contenedor.
  ...
  _injectStyles() {
    ...
    s.textContent = `
      /* GPS — ahora vive en #wp-side-slot-3 (panel lateral), no aquí.
         Activo: el anillo se queda neutro, solo el punto central se pone
         verde y pulsa. */
      #wp-side-slot-3.loading .wp-gps-dot,
      #wp-side-slot-3.active .wp-gps-dot { animation: gpsPulse 1.2s infinite; }
      #wp-side-slot-3.active .wp-gps-dot { fill: #16a34a; }
