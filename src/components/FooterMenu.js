...
// Footer menu con animación spring — Actividades | 📍 Home | Social
// ====================================================================
...
export class FooterMenu {
  ...
  _injectStyles() {
    ...
    s.textContent = `
      ...
      .wp-menu-item.active .wp-menu-icon { color: #2563eb; }
      .wp-menu-item.active .wp-menu-label { color: #2563eb; }
  ...
  _build() {
    ...
    wrap.innerHTML = `
      <div class="wp-menu-bar">
        <!-- LEFT: Actividades -->
        <div class="wp-menu-side left">
          <div class="wp-menu-item" id="wpm-actividades">
            <div class="wp-menu-icon">${ICON_ACTIVIDADES}</div>
            <span class="wp-menu-label">Actividades</span>
          </div>
        </div>
        ...
        <!-- RIGHT: Social -->
        <div class="wp-menu-side right">
          <div class="wp-menu-item" id="wpm-social">
            <div class="wp-menu-icon">${ICON_SOCIAL}</div>
            <span class="wp-menu-label">Social</span>
          </div>
        </div>
      </div>
    `;
  ...
  _attachEvents() {
    ...
    // Actividades
    document.getElementById('wpm-actividades')?.addEventListener('click', () => {
      this._pulse('wpm-actividades');
    ...
    // Social
    document.getElementById('wpm-social')?.addEventListener('click', () => {
      this._pulse('wpm-social');
  ...
  _pulse(id) {
    ...
    window.gsap.timeline()
      ...
      .to(el, { scale: 1.1, duration: 0.22, ease: 'back.out(3)' })
  ...
  _setActive(id) {
    ...
    // Reset all
    ['wpm-actividades','wpm-social'].forEach(elId => {
    ...
    // Set active
    if (id !== 'home') {
      const el = document.getElementById('wpm-' + id);
      if (el) el.classList.add('active');
    }

    // Center button color — blue always for home, dimmed otherwise
    const centerBtn = document.querySelector('.wp-menu-center-btn');
    if (centerBtn) {
      centerBtn.style.background = id === 'home'
        ? 'linear-gradient(145deg, #3b82f6, #1d4ed8)'
        : 'linear-gradient(145deg, #9ca3af, #6b7280)';
      centerBtn.style.boxShadow = id === 'home'
  ...
  // Entrance animation
  animateIn() {
    const bar = document.querySelector('.wp-menu-bar');
    const center = document.getElementById('wpm-home');
    if (!bar || !window.gsap) return;
    window.gsap.fromTo(bar,
      { y: 80, opacity: 0 },
      { y: 0, opacity: 1, duration: 0.5, ease: 'back.out(1.5)', delay: 0.3 }
    );
    window.gsap.fromTo(center,
      { scale: 0, opacity: 0 },
      { scale: 1, opacity: 1, duration: 0.4, ease: 'back.out(2)', delay: 0.55 }
    );
  }
}
