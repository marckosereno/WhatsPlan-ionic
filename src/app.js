...
// ════════════════════════════════════════════════════════════════════
try {
  // Centrarlo verticalmente entre el topbar y el panel de categorías
  // (#map-results-panel, visible desde el HTML estático)
  ...
  // del panel lateral — feImage + feDisplacementMap. Solo lo aplica el navegador
  // si soporta backdrop-filter:url() (Chrome); el CSS ya trae fallback plano.
  // El mapa de desplazamiento se genera con la SDF de un rectángulo redondeado
  // (no un gradiente circular simple) para que calce con la forma real de la cápsula.
  const buildLiquidGlassFilter = () => {
    try {
      const bg = document.getElementById('wp-side-panel-bg');
      ...
      defs.innerHTML =
        '<filter id="wp-glass-filter" x="0" y="0" width="' + w + '" height="' + h + '" filterUnits="userSpaceOnUse" primitiveUnits="userSpaceOnUse">' +
      ...
      console.error('[wp-side-panel] buildLiquidGlassFilter falló, se queda con el glass plano de respaldo:', e);
    }
  };

  const tick = () => { updateSidePanelPosition(); buildLiquidGlassFilter(); };
